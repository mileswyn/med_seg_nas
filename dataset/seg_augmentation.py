...
def output_images(x, y, i, n_iter, k=1):
    """
    ...
    if k == 2:
        ...
        ax1.set_yticklabels([])
        plt.savefig(
            'E:\\CodeRepo\\myUnet\\temp\\testwhiletrain\\L_' + str(n_iter-1) + '_epoch_'
            + str(i))
...
def input_images(x, y, i, n_iter, k=1):
    ...
    if k == 1:
        ...
        ax1.set_yticklabels([])
        plt.savefig(
            'E:\\CodeRepo\\myUnet\\temp\\pred\\L_' + str(n_iter-1) + '_epoch_'
            + str(i))
...
class RandomElasticTransform_image(object):
    def __init__(self, alpha = 3, sigma=0.07, img_type='L'):
        self.alpha = alpha
        self.sigma = sigma
        self.img_type = img_type
    ...
    def _elastic_transform(self, img):
        ...
        return (Image.fromarray(img,mode=self.img_type))
...
class RandomElasticTransform_mask(object):
    def __init__(self, alpha = 3, sigma=0.07, img_type='L'):
        self.alpha = alpha
        self.sigma = sigma
        self.img_type = img_type

    def _elastic_transform(self, mask):
    ...
    def __call__(self, mask):
        ...
        """
        if random.random() < 0.5:
            return self._elastic_transform(mask)
        else:
            return (mask)
