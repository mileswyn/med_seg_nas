...
def december_data2array(base_path, foldchose, store_path,img_rows,img_cols):
...
def get_valid_list( base_path):
    fileList = os.listdir(os.path.join(base_path, 'TrainingData'))
    fileList = sorted([os.path.join(base_path, 'TrainingData',x) for x in fileList if '.mhd' in x and 'segmentation' not in x])
    num = [8,12,21,22,30,32,33,38,42,45]
    # fileList = [fileList[i] for i in num]
    fileList = [fileList[i] for i in num] # if i not in num]
    return fileList
...
class Promise12(data.Dataset):
    ...
    def __init__(self, root, mode):
        ...
        fold0 = [8,12,21,22,30,32,33,38,42,45]
        fold1 = [0,1,7,13,15,23,24,34,40,46]
        fold2 = [2,5,9,17,18,25,28,35,39,44]
        fold3 = [3,6,10,14,19,26,29,36,41,47]
        fold4 = [4,11,16,20,27,31,37,43,48,49]
        # SECOND
        # store data in the npy file
        data_path = os.path.join(root, 'December_npy')

        if not os.path.exists(data_path):
            create_exp_dir(data_path)
            december_data2array(root, fold0, data_path, 256, 256)
        else:
            print('read the data from: {}'.format(data_path))

        self.test_file_list = get_test_list(root)
        #self.validfilelist = get_valid_list(root)
        self.blank_layer = []
        ...
        if mode == 'train':
            ...
            # img_train = Image.fromarray(self.X_train,mode='F')
            # target_train = Image.fromarray(self.y_train,mode='L')
            # img_train,target_train = self.joint_transform(img_train,target_train)
            # self.X_train = np.array(img_train)
            # self.y_train = np.array(img_train)

            with open('/hdd1/wyn/DenseNAS/dataset/blank.pkl', 'rb') as f:
                a = pickle.load(f)
