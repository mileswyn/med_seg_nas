...
#                       kernel_size=1, stride=1, padding=0, bias=False),
#             nn.InstanceNorm3d(block_ch, affine=False, track_running_stats=True),
...
#         # promise12 :_num_classes=2
#         # self._num_classes = 2
#         # sliver07 :_num_classes=1 use different loss/ use sigmoid at last
#         self._num_classes = 2
#         self.initialize()
#
#     def initialize(self):
...
#         return alpha_params
#
#     def display_arch_params(self, display=False):
#         branch_weights = []
#         head_op_weights = []
#         stack_op_weights = []
...
#             for j in range(self.config.search_params.adjoin_connect_nums[stage]):  # 0-3
#                 out_index = i + j + 1
...
#         total_cost += cost
#         cost, block_data = cost_func(self.maxpool1, block_data)
#         cost_list.append(cost)
#         # block_datas.append(block_data)
#         total_cost += cost
#         cost, block_data = cost_func(self.input_block2, block_data)
#         cost_list.append(cost)
#         block_datas.append(block_data)
#         total_cost += cost
...
class Network(nn.Module):
    ...
    def get_cost_list(self, data_shape, cost_type='flops',
        ...
        for block_id, block in enumerate(self.blocks):
            ...
            #         stack_layer_flops.append(op_flops)
            # block_flops.append([head_branch_flops, stack_layer_flops])
            block_flops.append([head_branch_flops])
            block_datas.append(block_data)
