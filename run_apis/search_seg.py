...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', metavar='C',
                        default='/hdd1/wyn/DenseNAS/configs/abdominal_search_cfg_resnet.yaml',
                        help='The Configuration file')
    ...
    scheduler = get_lr_scheduler(config, search_optim.weight_optimizer, 200) # set iteration 200
    scheduler.last_step = start_epoch * (200 // config.data.batch_size + 1)

    search_trainer = SearchTrainer(search_optim, criterion, scheduler, config, args, writer, fold=3, batch_size=config.data.batch_size)
    ...
    for epoch in range(start_epoch, config.train_params.epochs):
        ...
        if epoch >= config.search_params.arch_update_epoch:  # epoch>=50
            ...
            logging.info('EPOCH%d Arch Train_metric mIoU %.5f dice %.5f loss %.4f %s %.2f',
                         epoch, train_mIoU, train_Dice, train_loss2, config.optim.sub_obj.type, sub_obj)
        ...
        logging.info('EPOCH%d Weights Train_metric  mIoU %.5f dice %.5f loss %.4f sub_obj_train "%s %.2f"',
                     epoch, train_mIoU, train_Dice, train_loss2, config.optim.sub_obj.type, sub_obj_train)
        ...
        if epoch >= config.search_params.val_start_epoch:
            ...
            logging.info('EPOCH%d Valid_metric  val_mIoU %.5f val_dice %.5f loss %.4f sub_obj_val "%s %.2f"',
                         epoch, val_mIoU, val_Dice, train_loss2, config.optim.sub_obj.type, sub_obj_val)
